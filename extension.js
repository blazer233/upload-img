...
const COMMIT = 'image update commit'
...
const gitHandle = async (uploadUrl, filePaths, folder, progress) => {
    try {
        ...
        if (!isRemove) return
        progress.report({ increment: 20, message: "文件已拷贝到指定目录" });
        await execFile('git pull', { cwd: uploadUrl });
        progress.report({ increment: 40, message: "git已更新" });
        await execFile('git add .', { cwd: uploadUrl });
        progress.report({ increment: 60, message: "文件已添加" });
        await execFile(`git commit -m ${COMMIT}`, { cwd: uploadUrl });
        progress.report({ increment: 80, message: "文件已提交" });
        await execFile('git push', { cwd: uploadUrl });
        progress.report({ increment: 100, message: "文件已上传" });
